#!/usr/bin/env python
"""
Rotation-centre sweep for Y350a_HT -- step 5 only, one slice per
candidate centre.
...
There is no local_run.sh here -- this sample is set up for Polaris, and
polaris_run.sh's `python $SCRIPT $CONFIG` line passes nothing after the config,
so append the sweep flags to that line by hand, or run the mpirun above
...
Caveat, and it applies to this dataset: step 4 writes `pdata{k}_{bin}` after an
inter-distance amplitude match whose patch means are taken at the config's
centre, so with ndist > 1 those arrays carry a weak, low-frequency dependence on
it.  This sweep re-stitches from them without redoing that match.  The effect is
smooth and the double-edge signal it competes with is sharp, so the sweep still
brackets the centre well -- but confirm the winner with one full steps15 step-5
run before committing to the bin-0 job.  (AtomiumL1_largedisp, where this script
came from, has ndist == 1 and no such caveat.)
"""
