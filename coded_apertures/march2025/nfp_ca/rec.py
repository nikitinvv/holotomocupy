...
class Rec:
    ...
    def plot_debug(self, vars, etas, top, bottom, alpha, d, dref, i):
        ...
        if i % self.vis_step == 0 and self.vis_step != -1 and self.show:
            ...
            for k in range(0, npp * 2):
                ...
                errt[k] = self.minF(tmp, d)+ self.lam*cp.linalg.norm(self.C(self.G(qt)))**2 + cp.linalg.norm(cp.abs(self.D(qt[cp.newaxis]))-dref)**2

            t = alpha * (cp.arange(2 * npp)) / (npp - 1)
            errt2 = self.minF(self.fwd(ri,r,psi,q), d) + self.lam*cp.linalg.norm(self.C(self.G(q)))**2+ cp.linalg.norm(cp.abs(self.D(q[cp.newaxis]))-dref)**2
    ...
    def error_debug(self,vars, reused, d, dref, i):
        ...
        if i % self.err_step == 0 and self.err_step != -1:
            err = self.minF(reused["big_psi"], d)+ self.lam*cp.linalg.norm(self.C(self.G(q)))**2 + cp.linalg.norm(cp.abs(self.D(q[cp.newaxis]))-dref)**2
