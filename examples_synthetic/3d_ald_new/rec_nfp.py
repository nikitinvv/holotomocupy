...
# ID16a setup
ndist = int(sys.argv[1])
z1s = 0#float(sys.argv[1])
...
def line_search(minf, gamma, fu, fu0, fd, fd0):
    """ Line search for the step sizes gamma"""
    while (minf(fu, fu0)-minf(fu+gamma*fd, fu0+gamma*fd0) < 0 and gamma >= 1/64/ndist):
        gamma *= 0.5
    if (gamma < 1/64/ndist):  # direction not found
        # print('no direction')
        gamma = 0
    return gamma
...
pars = {'niter': 513, 'upd_psi': True, 'upd_prb': True,
        'err_step': 16, 'vis_step': 64, 'gammapsi': 0.5/ndist, 'gammaprb': 0.5}
...
write_tiff(np.abs(rec),f'/data/vnikitin/rec_abs_new{flg}')
write_tiff(np.angle(rec),f'/data/vnikitin/rec_angle_new{flg}')
write_tiff(np.angle(rec_prb),f'/data/vnikitin/rec_prb_angle_new{flg}')
write_tiff(np.abs(rec_prb),f'/data/vnikitin/rec_prb_abs_new{flg}')
