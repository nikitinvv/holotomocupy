...
class Rec:
    ...
    def BH(self, d, vars):
        ...
        rho = self.rho
        reused = {}
        print(f"0 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
        for i in range(self.niter):
            t =time.time()
            self.calc_reused(reused, vars)            
            print(f"1 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            self.gradientF(reused, d)
            print(f"2 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            self.error_debug(vars, reused, d, i)
            print(f"3 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            self.vis_debug(vars, i)
            print(f"4 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            grads = self.gradients(vars, reused)
            print(f"5 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            ...
            print(f"6 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            alpha, top, bottom = self.calc_alpha(vars, grads, etas, reused, d)
            print(f"7 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            grads = []
            # debug approxmation
            self.plot_debug(vars, etas, top, bottom, alpha, d, i)
            print(f"8 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            ...
            print(f"9 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            vars["psi"][:] = self.R(vars["u"])
            vars["psi"][:] = self.expR(vars['psi'])
            print(f"10 Total memory used by process: {process.memory_info().rss / (1024 ** 2):.2f} MB")
            print(time.time()-t)            
        return vars
    ...
    def gradient_q(self, r, psi, gradF):
        flg = chunking_flg(locals().values())        
        if flg:
            gradq = []
        ...
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)                
        def _gradient_q(self, gradq, r, psi, gradF):
            for j in range(self.ndist):# dirct writing in 
                mspsi = self.M(self.S(r[:,j], psi),j)            
                gradq[j] += cp.sum(cp.conj(mspsi)*gradF[:,j],axis=0)        
        _gradient_q(self, gradq, r, psi, gradF)
    ...
    def gradients(self, vars, reused):
        ...
        dq = self.gradient_q(r, psi, gradF)
    ...
    def calc_reused(self, reused, vars):
        ...
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)
        def _calc_reused2(self, big_psi, r, psi, q):
            for j in range(self.ndist):
                mspsi = self.M(self.S(r[:,j], psi),j)  
                big_psi[:,j] = self.D(mspsi*q[j],j) 

        # _calc_reused1(self, mspsi, psi, r)
        _calc_reused2(self, big_psi, r, psi, q)
    ...
    def calc_beta(self, vars, grads, etas, reused, d):
        ...
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)
        def _calc_beta(self, res,  r, big_psi, gradF, dr1, dr2, psi, Rdu1, Rdu2, d, q, dq1, dq2):            
        ...
        _calc_beta(self, res,  r, big_psi, gradF, dr1, dr2, psi,Rdu1, Rdu2, d, q, dq1, dq2)  
        if flg:
            for k in range(1,len(res)):
                res[0][0] += res[k][0]
                res[0][1] += res[k][1]
    ...
    def calc_alpha(self, vars, grads, etas, reused, d):
        ...
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)
        def _calc_alpha(self, res,  r, big_psi, gradF, dr1, dr2, psi, Rdu2, d, q, dq2):            
        ...
        _calc_alpha(self, res,  r, big_psi, gradF, dr1, dr2, psi, Rdu2, d, q, dq2)
        if flg:
            for k in range(1,len(res)):
                res[0][0] += res[k][0]
                res[0][1] += res[k][1]                
