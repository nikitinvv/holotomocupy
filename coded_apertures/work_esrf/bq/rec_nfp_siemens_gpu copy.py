...
sx0 = 1.286e-3
z1 = np.tile(float(sys.argv[2])*1e-3-sx0, [npos]) # 5.5
...
def LTop(data):
    ...
    for j in range(npos):
        ...
        psir = GT(psir, wavelength, voxelsize, distances[j],'constant')        
        psi[:,j] = psir
    return psi
...
arr1 = cp.pad(tmp[:,0],((0,0),(ne//2-n//2,ne//2-n//2),(ne//2-n//2,ne//2-n//2)),'constant')     
...
data = cp.array(data)
pars = {'niter': 2049, 'err_step': 8, 'vis_step': 32}
