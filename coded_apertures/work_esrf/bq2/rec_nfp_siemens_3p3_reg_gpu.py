...
def DT(psi,x,dx):
    ...
    [xi2, xi1] = cp.meshgrid(xi1, xi1)
    for j in range(npos):
        res[:,j] = psi.copy()#cp.pad(psi, ((0, 0), (ne//2, ne//2), (ne//2, ne//2)))
        xj = cp.array(x[:,j,:,np.newaxis,np.newaxis])
        ...
        res[:,j] = cp.fft.ifft2(pp*xiall*cp.fft.fft2(res[:,j]))   

        res[:,j] = -2*np.pi*1j*res[:,j]#[:, ne//2:-ne//2, ne//2:-ne//2]       
    return res
...
def D2M(psi,q,x,dpsi1,dq1,dx1,dpsi2,dq2,dx2,gradF):    
    res =  redot(gradF,q*DT(dpsi1,x,dx2))
    res += redot(gradF,q*DT(dpsi2,x,dx1))
    res += redot(gradF,q*D2T(psi,x,dx1,dx2))  
    res += redot(gradF,dq1*Sop(dpsi2,x))
    res += redot(gradF,dq2*Sop(dpsi1,x) )
    res += redot(gradF,dq1*DT(psi,x,dx2))
    res += redot(gradF,dq2*DT(psi,x,dx1))    
    return res
...
def calc_beta(vars,grads,etas,d,gradF):
    ...
    top = D2M(psi,q,x,dpsi1,dq1,dx1,dpsi2,dq2,dx2,gradF)
    bottom = D2M(psi,q,x,dpsi2,dq2,dx2,dpsi2,dq2,dx2,gradF)    
...
def plot_debug2(vars,etas,top,bottom,alpha,data):
    ...
    for k in range(0,npp*2):
        ...
        gpsit = Cop(Gop(psit))
        errt[k] = np.linalg.norm(fpsit)**2+ lam[0]*np.linalg.norm(gpsit)**2
        
    t = alpha*(cp.arange(2*npp))/(npp-1)
    tmp = np.abs(Lop(Sop(psi,x)*q))-data
    gpsi = Cop(Gop(psi))
    errt2 = np.linalg.norm(tmp)**2+lam[0]*np.linalg.norm(gpsi)**2
...
def err_debug(vars, grads, data):    
    ...
    tmp = np.abs(Lop(Sop(psi,x)*q))-data
    gpsi = Cop(Gop(psi))
    err = np.linalg.norm(tmp)**2+lam[0]*np.linalg.norm(gpsi)**2
...
vars = {}
vars['psi'] = cp.asarray(rec_paganin)
vars['prb'] = cp.ones([1,n+2*pad,n+2*pad],dtype='complex64')
vars['shift'] = cp.array(shifts)
vars['rho'] = [1,1,0.1]
vars['lam'] = [lam1,lam2]
data_rec = cp.sqrt(cp.asarray(data))#p.pad(cp.array(data),((0,0),(0,0),(pad,pad),(pad,pad)))

pars = {'niter': 10000, 'err_step': 64, 'vis_step': 128}
