...
parser.add_argument('--n', type=int, default=None,
                    help='Override detector crop size (images read as [...,:n,:n]); auto-detected from EDF if omitted')
parser.add_argument('--nobj', type=int, default=None,
                    help='Override stitched object size; auto-computed from n and magnifications if omitted')
...
if start_step > 4:
    ...
    with h5py.File(fpath, 'a', driver='mpio', comm=comm) as fid:
        ...
        for j in local_ids:
            ...
            if j % 100 == 0:
                logger.debug(f'step4: proj {int(j):4d}/{ntheta}')
    ...
    for bin in range(nlevels):
        ...
        with h5py.File(fpath) as fid, \
            ...
            for i, j in enumerate(local_ids):
                ...
                for k in range(ndist):
                    pj_ds[k][j] = pj[k].get()
                phase = multiPaganin(pj, distances / norm_magnifications**2, wavelength, voxelsize_bin, paganin, 1e-5)
