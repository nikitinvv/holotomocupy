...
Launch with:
    mpirun -n <N> python step12_mpi.py /path/to/scan/ScanPrefix
    mpirun -n <N> python step12_mpi.py /path/to/scan/ScanPrefix --start-step 2
    mpirun -n <N> python step12_mpi.py /path/to/scan/ScanPrefix --start-step 5 --paganin 120
"""
...
parser = argparse.ArgumentParser(
    description='Steps 1–5 for holotomography: convert, preprocess, shifts, binned, Paganin+FBP.',
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)
parser.add_argument('--pfile', type=str,
                    help='Full path to the scan prefix, e.g. '
                         '/data2/vnikitin/atomium/20240924/AtomiumS2/AtomiumS2_HT_007nm')
parser.add_argument('--start-step', type=int, default=1, choices=[1, 2, 3, 4, 5],
                    help='Step to start from (1=convert, 2=preprocess, 3=shifts, 4=binned, 5=paganin+fbp)')
parser.add_argument('--rotation-center-shift', type=float, default=0.0,
                    help='Rotation centre shift in pixels (applied to column shifts in steps 4-5)')
parser.add_argument('--nlevels', type=int, default=4,
                    help='Number of binning levels written in step 4')
parser.add_argument('--paganin', type=float, default=120.0,
...
pfile = os.path.basename(args.pfile.rstrip('/'))
path  = os.path.dirname(args.pfile.rstrip('/')) + '/'
...
# Stitched object size (same at all steps that use it: 4, 5)
...
if start_step > 2:
    ...
    if rank == 0:
        ...
        with h5py.File(fpath) as fid:
            for k in range(ndist):
                ...
                dark_arr[:, k] = fid[f'/exchange/data_dark{k}'][:]
    ...
    if rank == 0:
        ...
        with h5py.File(fpath) as fid:
            for k in range(ndist):
                data = cp.array(fid[f'/exchange/data{k}'][0].astype('float32'))
                data -= dark_gpu[k]
                data[data < 0] = 0
...
if start_step > 4:
    ...
    if rank == 0:
        with h5py.File(fpath) as fid:
            ref = fid['/exchange/pref'][:].astype('float32')          # [ndist, n, n]
        ...
        for j in local_ids:
            ...
            for t in range(2):
                ...
                for k in range(ndist - 1, -1, -1):
                    ...
                    padx0 = min(nobj, max(0, padx0)) + 2
                    pady0 = min(nobj, max(0, pady0)) + 2
                    padx1 = min(nobj, max(0, padx1)) + 2
                    pady1 = min(nobj, max(0, pady1)) + 2
                ...
                st, end = nobj // 4 + nobj // 8, 3 * nobj // 4 - nobj // 8
                mmean = (cp.mean(srdata[:, st:end, st:end], axis=(1, 2))
                         / cp.mean(srdata[0, st:end, st:end]))
                data /= mmean[..., None, None]

            if j % 100 == 0:
                logger.debug(f'step4: proj {int(j):4d}/{ntheta}, mmean={mmean.get()}')
...
if start_step > 5:
    ...
    for bin in range(nlevels):
        ...
        cl_tomo = Tomo(nobj_bin, nchunk, theta, mask_r=1.1)
