...
if start_step > 5:
    ...
    for bin in range(start_level_rec, nlevels):
        ...
        # TEMPORARY: pin the placement to the values the sweep settled on,
        # rather than whatever estimate_overlap measures this run. Applied
        # *before* tile_offsets is written, because step 6 reads
        # cshifts_final + tile_offsets and must see the same placement the
        # stitch below actually used — otherwise it would start from an object
        # that does not match its own positions. (v, h) in object px on the
        # finest grid.
        ...
        # --- Record the placement next to cshifts_final ----------------------
        ...
        # Rewritten once per bin, so the finest placement is what survives, and
        # written *before* any HOLO_TILE_ERR perturbation — a sweep run must not
        # leave its deliberate error on disk.
        if rank == 0:
