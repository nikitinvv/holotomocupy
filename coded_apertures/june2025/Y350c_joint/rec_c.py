...
class Rec:
    def __init__(self, args):
        ...
        nbytes = 24 * args.nchunk * args.npsi**2* np.dtype("complex64").itemsize            
        ...
        self.fkerp = [[[] for _ in range(args.ndist)] for _ in range(args.ngpus)]
        ...
        for igpu in range(args.ngpus):
            with cp.cuda.Device(igpu):
                ...
                for j in range(args.ndist):
                    self.fkerp[igpu][j] = cp.exp(-1j * cp.pi * args.wavelength * args.distancep[j] * (fx**2 + fy**2))
    ...
    def Dp(self, psi,j):
        ...
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)
        def _Dp(self, big_psi, psi):
            ff = self._fwd_pad(psi)
            ff *= 2        
            ff = cp.fft.ifft2(cp.fft.fft2(ff)*self.fkerp[cp.cuda.Device().id][j])
            ff = ff[:,self.nq//2:-self.nq//2,self.nq//2:-self.nq//2]            
            big_psi[:] = ff[:, self.pad : self.nq - self.pad, self.pad : self.nq - self.pad]
                    
        _Dp(self, big_psi, psi)
        return big_psi
    ...
    @timer
    def gradient_u(self, r, psi, d, q, u):
        ...
        def _gradient_psi(self, gradpsi, r,  psi, d, q):            
            for j in range(self.ndist):
                ...
                big_psi = self.D(mspsi*self.Dp(q,j),j) 
                td = d[:,j] * (big_psi / (cp.abs(big_psi)))
                gradF = self.DT(2 * (big_psi - td), j)
                tmp = self.MT(cp.conj(self.Dp(q,j)) * gradF,j)
                if j==0:
        ...
        if self.lam>0:
            gg =self.G(u)
            gg = self.G(gg)
            self.linear_batch(grad_u,gg,1,2*self.lam)
    ...
    def gradient_q(self, r, psi, d, q):
        ...
        if flg:
            ...
            for igpu in range(self.ngpus):
                with cp.cuda.Device(igpu):
                    gradq.append(cp.zeros([1, self.nq, self.nq], dtype="complex64"))
        else:
            gradq = cp.zeros([1, self.nq, self.nq], dtype="complex64")
        ...
        def _gradient_q(self, gradq, r, psi, d, q):
            for j in range(self.ndist):# dirct writing in 
                ...
                td = d[:,j] * (big_psi / (cp.abs(big_psi)))
                gradF = self.DT(2 * (big_psi - td), j)
                gradq += cp.sum(self.DpT(cp.conj(mspsi)*gradF,j),axis=0)        
    ...
    def gradient_r(self,  r, psi, d, q):
        ...
        def _gradient_r(self, gradr,  r, psi, d, q):
            ...
            for j in range(self.ndist):
                ...
                dm1 = self.Dp(q,j)*self.M(dt1,j)
                dm2 = self.Dp(q,j)*self.M(dt2,j)
                mspsi = self.M(self.S(r[:,j], psi),j)  
                big_psi = self.D(mspsi*self.Dp(q,j),j) 
                td = d[:,j] * (big_psi / (cp.abs(big_psi)))
                gradF = self.DT(2 * (big_psi - td), j)
    ...
    def gradients(self, vars, d):
        (u, q, psi,  r) = (vars["u"], vars["q"], vars["psi"], vars["r"])
        du = self.gradient_u(r, psi, d, q, u)
    ...
    def calc_beta(self, vars, grads, etas, d):
        ...
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)
        def _calc_beta(self, res,  r, dr1, dr2, psi, Rdu1, Rdu2, d, q, dq1, dq2):            
            ...
            for j in range(self.ndist):
                ...
                mspsi = self.M(self.S(r[:,j], psi),j)  
                dv1 = self.Dp(q,j) * self.M(sdpsi1R+dt1,j)+self.Dp(dq1,j)*mspsi#[:,j]
                dv2 = self.Dp(q,j) * self.M(sdpsi2R+dt2,j)+self.Dp(dq2,j)*mspsi#[:,j]
                
                d2v1 = self.Dp(q,j)*self.M(2*sdpsi1RR+dt12+dt1R+d2t1,j)
                d2v1 += self.Dp(dq1,j)*self.M(sdpsi2R+dt2,j)
                d2v1 += self.Dp(dq2,j)*self.M(sdpsi1R+dt1,j)

                d2v2 = self.Dp(q,j)*self.M(2*sdpsi2RR+2*dt2R+d2t2,j)
                d2v2 += 2*self.Dp(dq2,j)*self.M(sdpsi2R+dt2,j)
        ...
        _calc_beta(self, res,  r, dr1, dr2, psi,Rdu1, Rdu2, d, q, dq1, dq2)  
        if flg:
            for k in range(1,len(res)):
                res[0][0] += res[k][0]
                res[0][1] += res[k][1]
        ...
        if self.lam>0:
            step = self.npsi//8
            for k in range(self.npsi//step):
                st = k*step
                end = (k+1)*step
                st1 = max(st-4,0)
                end1 = min(end+4,self.npsi)
                gu1 = self.G(du1[st1:end1])[st-st1:end1-st1-(end1-end)]
                gu2 = self.G(du2[st1:end1])[st-st1:end1-st1-(end1-end)]
                res[0] += 2*self.lam*self.redot_batch(gu1,gu2)
                res[1] += 2*self.lam*self.redot_batch(gu2,gu2)
        
                # a += 2*self.lam*self.redot_batch(gu1,gu2)
                # b += 2*self.lam*self.redot_batch(gu2,gu2)
        # print(a,b)
        # gu1 = self.G(du1)#[st1:end1])[st-st1:end1-st1-(end1-end)]
        # gu2 = self.G(du2)#[st1:end1])[st-st1:end1-st1-(end1-end)]            
        # res[0] += 2*self.lam*self.redot_batch(gu1,gu2)
    ...
    def calc_alpha(self, vars, grads, etas, d):
        (q, psi,  r) = (vars["q"], vars["psi"], vars["r"])
        (du1, dq1, dr1) = (grads["u"], grads["q"], grads["r"])
        ...
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)
        def _calc_alpha(self, res,  r, dr1, dr2, psi, Rdu2, d, q, dq2):            
        ...
        _calc_alpha(self, res,  r, dr1, dr2, psi, Rdu2, d, q, dq2)
        if flg:
            for k in range(1,len(res)):
                res[0][0] += res[k][0]
                res[0][1] += res[k][1]                
    ...
    def fwd(self,r,u,q):
        ...
        def _fwd(self,res,r,psi,q):            
            for j in range(self.ndist):
                mspsi = self.M(self.S(r[:,j],psi),j)
                res[:,j] = self.D(mspsi * self.Dp(q,j),j)
    ...
    def error_debug(self,vars, d, i):
        ...
        if i % self.err_step == 0 and self.err_step != -1:
            ...
            if self.lam>0:
                err += self.lam*self.minFL(self.G(u))
