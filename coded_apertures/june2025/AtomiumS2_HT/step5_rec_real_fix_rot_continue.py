...
args.path_out = f"{path_out}/{pfile}/rec_final4_continue_{ndist}_{rotation_axis}_{lam}_{rhoq}_{rhor}"
args.show = False
...
for level in range(nlevels):    
    ...
    if level==0:
        vars = {}                        
        iter = 0
        path = '/data/vnikitin/ESRF/ID16A/brain_rec/20240515/Y350c/Y350c_HT_015nm/rec_final4_4_1763.0_50.0_50.0_20.0/'
        r = np.load(f'{path}/r{iter:04}.npy')        
        q = np.ones([ndist,args.nq,args.nq],dtype='complex64')  
        for k in range(ndist):      
            q_angle = read_tiff(f'{path}/rec_prb_angle{k}/{iter:04}.tiff')
            q_abs = read_tiff(f'{path}/rec_prb_abs{k}/{iter:04}.tiff')
            q[k] = q_abs*np.exp(1j*q_angle)  
            q_abs=[]
            q_angle=[]
        u = np.ones([args.npsi,args.npsi,args.npsi],dtype='complex64')  
        for k in range(ndist):      
            u_re = read_tiff(f'{path}/rec_u_real/{iter:04}.tiff')
            u_im = read_tiff(f'{path}/rec_u_imag/{iter:04}.tiff')
            u = u_re+1j*u_im
            u_re=[]
            u_im=[]            
    else:
