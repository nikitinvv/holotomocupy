...
sys.path.insert(0, '..')
from cuda_kernels_double import *
from utils import *
...
class Rec:
    def __init__(self, args):
        ...
        nbytes = (
            16 * nchunk * npatch * npatch * np.dtype("complex128").itemsize            
        )
        ...
        for igpu in range(ngpus):
            with cp.cuda.Device(igpu):
                ...
                fx = cp.fft.fftfreq(nq * 2, d=voxelsize).astype("float64")
                [fx, fy] = cp.meshgrid(fx, fx)
                self.fker[igpu] = cp.exp(-1j * cp.pi * wavelength * distance * (fx**2 + fy**2)).astype('complex128')
                self.fkerc[igpu] = cp.exp(-1j * cp.pi * wavelength * distancec * (fx**2 + fy**2)).astype('complex128')
    ...
    def redot_batch(self,x,y):
        ...
        if flg:
            ...
            for igpu in range(self.ngpus):
                with cp.cuda.Device(igpu):
                    res.append(cp.zeros(1, dtype="float64"))
        else:
            res = cp.zeros(1, dtype="float64")
    ...
    def rec_tomo(self,d,rpsi,niter=1):
        ...
        u = np.zeros([d.shape[1],self.npsi,self.npsi],dtype='complex128')
    ...
    def initR(self, n):
        ...
        t = cp.linspace(-1/2, 1/2, n, endpoint=False).astype('float64')
        [dx, dy] = cp.meshgrid(t, t)
        phi = cp.exp((mu * (n * n) * (dx * dx + dy * dy)).astype('float64')) * (1-n % 4)
    ...
    def R(self, obj):
        ...
        # memory for result
        if flg:
            data = np.empty([self.ntheta, self.npsi,self.npsi], dtype="complex128")
        else:
            data = cp.empty([self.ntheta, self.npsi,self.npsi], dtype="complex128")
        
        @gpu_batch(self.nchunk, self.ngpus, axis_out=1, axis_inp=0)
        def _R(self, data, obj):
            
            [nz, n, n] = obj.shape
            theta = cp.array(self.theta, dtype='float64')
            m, mu, phi, c1dfftshift, c2dfftshift = self.initR(n)

            sino = cp.zeros([self.ntheta,nz,  n], dtype='complex128')
            ...
            t = cp.fft.fftfreq(n).astype('float64')
            w = cp.exp(-2*cp.pi*1j*t*(self.rotation_axis + n/2)).astype('complex128')
            sino = cp.fft.ifft(w*cp.fft.fft(sino))
            # normalization for the unity test
            sino /= cp.float64(4*n)#*np.sqrt(n*self.ntheta))                
    ...
    def RT(self, data):
        ...
        # memory for result
        if flg:
            obj = np.empty([self.npsi, self.npsi,self.npsi], dtype="complex128")
        else:
            obj = cp.empty([self.npsi, self.npsi,self.npsi], dtype="complex128")
        ...
        def _RT(self, obj, data):
            ...
            mua = cp.array([mu], dtype='float64')
            fde = cp.zeros([nz, 2*m+2*n, 2*m+2*n], dtype='complex128')
            ...
            fde /= cp.float64(n)#*np.sqrt(n*ntheta))  # normalization for the unity test
    ...
    def Spsi(self, psi, r):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            res = np.empty([len(r), self.nq, self.nq], dtype="complex128")
        else:
            res = cp.empty([len(r), self.nq, self.nq], dtype="complex128")
        
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)
        def _Spsi(self,res, psi,r):
            """Extract patches with subpixel shift"""

            x = cp.fft.fftfreq(self.nq).astype("float64")
            [y, x] = cp.meshgrid(x, x)
            tmp = cp.exp(
    ...
    def expR(self, psi):
        ...
        # memory for result
        if flg:
            epsi = np.empty([self.ntheta, self.npsi,self.npsi], dtype="complex128")
        else:
            epsi = cp.empty([self.ntheta, self.npsi,self.npsi], dtype="complex128")
    ...
    def logR(self, psi):
        ...
        # memory for result
        if flg:
            lpsi = np.empty([self.ntheta, self.npsi,self.npsi], dtype="complex128")
        else:
            lpsi = cp.empty([self.ntheta, self.npsi,self.npsi], dtype="complex128")
    ...
    def E(self, ri, code):
        ...
        # memory for result
        if flg:
            patches = np.empty([len(ri), self.npatch, self.npatch], dtype="complex128")
        else:
            patches = cp.empty([len(ri), self.npatch, self.npatch], dtype="complex128")
    ...
    def ET(self, patches, ri):
        ...
        if flg:
            ...
            for igpu in range(self.ngpus):
                with cp.cuda.Device(igpu):
                    code.append(cp.zeros([self.ncode, self.ncode], dtype="complex128"))
        else:
            code = cp.zeros([self.ncode, self.ncode], dtype="complex128")
    ...
    def S(self, ri, r, code):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            patches = np.empty([len(ri), self.nq, self.nq], dtype="complex128")
        else:
            patches = cp.empty([len(ri), self.nq, self.nq], dtype="complex128")
        ...
        def _S(self, patches, ri, r, code):
            ...
            x = cp.fft.fftfreq(self.npatch).astype("float64")
            [y, x] = cp.meshgrid(x, x)
            tmp = cp.exp(
                -2 * cp.pi * 1j * (y * r[:, 1, None, None] + x * r[:, 0, None, None])
            ).astype("complex128")
    ...
    def _fwd_pad(self,f):
        """Fwd data padding"""
        [ntheta, n] = f.shape[:2]
        fpad = cp.zeros([ntheta, 2*n, 2*n], dtype='complex128')
    ...
    def _adj_pad(self, fpad):
        ...
        n //= 2
        f = cp.zeros([ntheta, n, n], dtype='complex128')
    ...
    def D(self, psi):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            big_psi = np.empty([len(psi), self.n, self.n], dtype="complex128")
        else:
            big_psi = cp.empty([len(psi), self.n, self.n], dtype="complex128")
        
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)
        def _D(self, big_psi, psi):
            ff = psi.copy()
            ff = self._fwd_pad(ff)
            v = cp.ones(2*self.nq,dtype='float64')
    ...
    def DT(self, big_psi):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            psi = np.empty([len(big_psi), self.nq, self.nq], dtype="complex128")
        else:
            psi = cp.empty([len(big_psi), self.nq, self.nq], dtype="complex128")
    ...
    def Dc(self, psi):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            big_psi = np.empty([len(psi), self.nq, self.nq], dtype="complex128")
        else:
            big_psi = cp.empty([len(psi), self.nq, self.nq], dtype="complex128")
    ...
    def G(self,psi):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            res = np.empty([len(psi), self.nq, self.nq], dtype="complex128")
        else:
            res = cp.empty([len(psi), self.nq, self.nq], dtype="complex128")
        
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)
        def _G0(self, res, psi):
            stencil = cp.array([1, -2, 1]).astype('float64')            
    ...
    def gradientF(self, reused, d):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            gradF = np.empty([len(big_psi), self.nq, self.nq], dtype="complex128")
        else:
            gradF = cp.empty([len(big_psi), self.nq, self.nq], dtype="complex128")
    ...
    def gradient_u(self, scode, gradF, psi, rpsi, u, q):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            gradpsi = np.empty([self.ntheta,self.npsi, self.npsi], dtype="complex128")
        else:
            gradpsi = cp.empty([self.ntheta,self.npsi, self.npsi], dtype="complex128")
    ...
    def gradient_q(self, scode, gradF, psi):
        ...
        if flg:
            ...
            for igpu in range(self.ngpus):
                with cp.cuda.Device(igpu):
                    gradq.append(cp.zeros([self.nq, self.nq], dtype="complex128"))
        else:
            gradq = cp.zeros([self.nq, self.nq], dtype="complex128")
    ...
    def gradient_r(self, ri, r, psi, gradF, code, q):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            gradr = np.empty([len(ri), 2], dtype="float64")
        else:
            gradr = cp.empty([len(ri), 2], dtype="float64")
        
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)                               
        def _gradient_r(self, gradr, ri, r, psi, gradF, code, q):

            # frequencies
            xi1 = cp.fft.fftfreq(self.npatch).astype("float64")
    ...
    def calc_reused(self, reused, vars):
        ...
        if flg:
            scode = np.zeros([len(ri), self.nq, self.nq], dtype="complex128")
            big_psi = np.zeros([len(ri), self.n, self.n], dtype="complex128")
        else:
            scode = cp.zeros([len(ri), self.nq, self.nq], dtype="complex128")
            big_psi = cp.zeros([len(ri), self.n, self.n], dtype="complex128")
    ...
    def calc_beta(self, vars, grads, etas, reused, d):
        ...
        if flg:
            ...
            for igpu in range(self.ngpus):
                with cp.cuda.Device(igpu):
                    res.append(cp.zeros([2], dtype="float64"))
        else:
            res = cp.zeros([2], dtype="float64")
        ...
        def _calc_beta(self, res, ri, r, scode, big_psi, gradF, dr1, dr2, psi,Rdu1, Rdu2, d, q, dq1, dq2, code):            
            ...
            Ddn1 = self.D(dn1)
            Ddn2 = self.D(dn2)
            print(f'{cp.linalg.norm(d2n1)=}')
            print(f'{cp.linalg.norm(Ddn1)=}')
            print(f'{cp.linalg.norm(Ddn2)=}')
        ...
        top = cp.float64((res[0]).get()) 
        bottom = cp.float64((res[1]).get()) 
        return top/bottom
    ...
    def fwd(self,ri,r,rpsi,code, u,q):
        ...
        flg = chunking_flg(locals().values())        
        if flg:
            res = np.empty([len(ri), self.n, self.n], dtype="complex128")
        else:
            res = cp.empty([len(ri), self.n, self.n], dtype="complex128")
