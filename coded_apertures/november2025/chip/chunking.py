        ...
        def inner(*args):
            ...
            for igpu in range(ngpus_adj):
                ...
                if np.prod(gout.shape) == 0:
                    break
                if axis_inp == 0:
                    ginp = [x[igpu * gsize : (igpu + 1) * gsize] for x in inp[: proper + cproper]]
                    if len(inp[proper + cproper :]) > 0:
                        ginp.extend(inp[proper + cproper :])

                if axis_inp == 1:
                    ginp = [
                        x[:, igpu * gsize : (igpu + 1) * gsize] for x in inp[: proper + cproper]
                    ]
                    if len(inp[proper + cproper :]) > 0:
                        ginp.extend(inp[proper + cproper :])
...
def run(
    ...
    out_pinned = np.frombuffer(pinned_mem, out.dtype, np.prod([2, *out_shape0])).reshape(
        [2, *out_shape0]
    )
    ...
    for j in range(2):  # do it twice to assign memory pointers
        for k in range(proper):
            ...
            inp_pinned[j].append(
                np.frombuffer(pinned_mem + offset, inp[k].dtype, np.prod(inp_shape0)).reshape(
                    inp_shape0
                )
            )
