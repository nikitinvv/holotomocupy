...
class Rec:
    ...
    def BH(self, d, vars):
        ...
        for i in range(self.niter):
            ...
            # calc alpha
            top = -(redot(grads["u"], etas["u"]) / self.rho[0] ** 2 + redot(grads["q"], etas["q"]) / self.rho[1] ** 2 + redot(grads["r"], etas["r"]) / self.rho[2] ** 2)
            bottom = self.hessian_new(vars, etas, etas)
            alpha = top / bottom
    ...
    def hessian_new2(self, vars, grads, etas):
        """Cascade hessian"""
    ...
    def F2(self, x):
        x31, x32, x33 = x
        x22 = cp.zeros([self.ntheta, self.ndist, self.npsi, self.npsi], dtype="complex64")
        ...
        for k in range(self.ndist):
            ...
            rix = cp.floor(rx).astype("int32")
            riy = cp.floor(ry).astype("int32")
            ...
            for iy in [0, 1]:
                for ix in [0, 1]:
                    ...
                    x22[:, k] += w[:, np.newaxis, np.newaxis] * ishift(x32,rix + ix, riy + iy)
    ...
    def dF2(self, x, y, return_x=True):
        ...
        if return_x:
            x22 = cp.zeros([self.ntheta, self.ndist, self.npsi, self.npsi], dtype="complex64")
        y22 = cp.zeros([self.ntheta, self.ndist, self.npsi, self.npsi], dtype="complex64")
        
        for k in range(self.ndist):
            # y22=T_{x32}(y33)+DT_{x32}|_{x33}(y33)
            rx = x33[:, k, 1]
            ry = x33[:, k, 0]
            rix = cp.floor(rx).astype("int32")
            riy = cp.floor(ry).astype("int32")
            for iy in [0, 1]:
                for ix in [0, 1]:
                    ...
                    # DT_{x32}|_{x33}(y33)
                    w = cp.pi / 2 * (cp.cos(cp.pi / 2 * dry) ** 2 * cp.sin(cp.pi * drx) * y33[:, k, 1] + cp.cos(cp.pi / 2 * drx) ** 2 * cp.sin(cp.pi * dry) * y33[:, k, 0])                
                    y22[:, k] += w[:, np.newaxis, np.newaxis] * ishift(x32,rix + ix, riy + iy)
    ...
    def d2F2(self, x, y, z):
        ...
        for k in range(self.ndist):
            ...
            # y22=DT_{y32}|_{x33}(z33)+DT_{z32}|_{x33}(y33)+D2T_{x32}|_{x33}(y33,z33)
            rx = x33[:, k, 1]
            ry = x33[:, k, 0]
            rix = cp.floor(rx).astype("int32")
            riy = cp.floor(ry).astype("int32")
            for iy in [0, 1]:
                for ix in [0, 1]:
                    ...
                    # D2T_{x32}|_{x33}(y33,z33)
                    w = (cp.pi**2 / 2) * (
                        -cp.cos(cp.pi / 2 * dry) ** 2 * cp.cos(cp.pi * drx) * y33[:, k, 1] * z33[:, k, 1]
                        - cp.cos(cp.pi / 2 * drx) ** 2 * cp.cos(cp.pi * dry) * y33[:, k, 0] * z33[:, k, 0]
                        + cp.sin(cp.pi * drx) * cp.sin(cp.pi * dry) * (y33[:, k, 1] * z33[:, k, 0] + y33[:, k, 0] * z33[:, k, 1])
                    )
                    y22[:, k] += w[:, np.newaxis, np.newaxis] * ishift(x32,rix + ix, riy + iy)

                    # DT_{z32}|_{x33}(y33)
                    w = cp.pi / 2 * (cp.cos(cp.pi / 2 * dry) ** 2 * cp.sin(cp.pi * drx) * y33[:, k, 1] + cp.cos(cp.pi / 2 * drx) ** 2 * cp.sin(cp.pi * dry) * y33[:, k, 0])
                    y22[:, k] += w[:, np.newaxis, np.newaxis] * ishift(z32,rix + ix, riy + iy)

                    if y is z:  # add the same
                        y22[:, k] += w[:, np.newaxis, np.newaxis] * ishift(z32,rix + ix, riy + iy)
                    else:
                        # DT_{y32}|_{x33}(z33)
                        w = cp.pi / 2 * (cp.cos(cp.pi / 2 * dry) ** 2 * cp.sin(cp.pi * drx) * z33[:, k, 1] + cp.cos(cp.pi / 2 * drx) ** 2 * cp.sin(cp.pi * dry) * z33[:, k, 0])
                        y22[:, k] += w[:, np.newaxis, np.newaxis] * ishift(y32,rix + ix, riy + iy)
    ...
    def gF2(self, x, y):
        ...
        x21, x22, x23, x24 = x  # q, u, r, psi
        dt1 = cp.empty([self.ntheta, self.npsi, self.npsi], dtype="complex64")
        dt2 = cp.empty([self.ntheta, self.npsi, self.npsi], dtype="complex64")        
        ...
        for k in range(self.ndist):
            ...
            rix = cp.floor(rx).astype("int32")
            riy = cp.floor(ry).astype("int32")
            y22k = cp.ascontiguousarray(y22[:, k])
            for iy in [0, 1]:
                for ix in [0, 1]:
                    # y32=T_{-y22}(x23)
                    drx = (ix - (rx - rix)).astype("float32")
                    dry = (iy - (ry - riy)).astype("float32")
                    w = (cp.cos(cp.pi / 2 * drx) * cp.cos(cp.pi / 2 * dry)) ** 2             
                    y32 += w[:, np.newaxis, np.newaxis] * ishift(y22k,rix + ix, riy + iy)
            ...
            for iy in [0, 1]:
                for ix in [0, 1]:
                    # y33=Re<y22,dT_{x24}(x23)>
                    drx = (ix - (rx - rix)).astype("float32")
                    dry = (iy - (ry - riy)).astype("float32")
                    w1 = cp.pi / 2 * (cp.cos(cp.pi / 2 * drx) ** 2 * cp.sin(cp.pi * dry))
                    w2 = cp.pi / 2 * (cp.cos(cp.pi / 2 * dry) ** 2 * cp.sin(cp.pi * drx))
                    dt1 += w1[:, np.newaxis, np.newaxis] * ishift(x24,rix + ix, riy + iy)
                    dt2 += w2[:, np.newaxis, np.newaxis] * ishift(x24,rix + ix, riy + iy)

            y33[:, k, 0] = redot(y22[:, k], dt1, axis=(1, 2))
            y33[:, k, 1] = redot(y22[:, k], dt2, axis=(1, 2))
