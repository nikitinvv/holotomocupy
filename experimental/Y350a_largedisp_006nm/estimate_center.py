...
class Scan:
    ...
    def __init__(self, dname, pfile, ntheta, nflat):
        self.dname, self.pfile, self.ntheta = dname, pfile, ntheta

        dark_f = sorted(glob.glob(f'{dname}/darkend*.edf'))[:nflat]
        ref0_f = sorted(glob.glob(f'{dname}/ref*_0000.edf'))[:nflat]
        ref1_f = sorted(glob.glob(f'{dname}/ref*_{ntheta:04d}.edf'))[:nflat]
        if not (dark_f and ref0_f):
            raise SystemExit(f'no darkend*/ref*_0000 EDFs in {dname}')
        ...
        self.shifts = np.loadtxt(f'{dname}/correct.txt', dtype='float32')[:ntheta]
    ...
    def frame(self, j):
        w = j / self.ntheta
        img = read_edf(f'{self.dname}/{self.pfile}_1_{j:04d}.edf') - self.dark
...
def main():
    ...
    ntheta = cfg.getint('ntheta', fallback=0) or int(
        max(int(f.split('_')[-1].split('.')[0])
            for f in glob.glob(f'{dname}/ref0000_*.edf')))

    print(f'scan     : {dname}')
    print(f'ntheta   : {ntheta}')
    sc = Scan(dname, pfile, ntheta, args.nflat)
    n = sc.n
    print(f'frame    : {n} x {n}')
