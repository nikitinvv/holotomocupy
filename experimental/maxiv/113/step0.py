#!/usr/bin/env python
"""
2D near-field ptychography reconstruction for a MAX IV / DanMAX scan.

Input:
  scan-NNNN.h5        - motor positions (tom_sam_x horizontal, tom_y vertical)
  scan-NNNN_orca.h5   - Orca detector data at entry/instrument/orca/data
  scan-(NNNN+1)_orca  - dark field (timescan)
  scan-(NNNN+2)_orca  - flat field (timescan)
...
DATA_SCAN  = 113
DARK_SCAN  = 115   # ~200 cts   (no beam)   -- swap of user's initial note
FLAT_SCAN  = 114   # ~7300 cts  (no sample)

# Beamline geometry (user-provided)
ENERGY                     = 19.55        # keV
...
N_POSITIONS     = 32                      # None = all frames; int = N closest to (0,0)
NITER           = 513
NCHUNK          = 4
RHO             = [1.0, 2.0, 0.1]         # object, probe, positions
CHECKPOINT_STEP = 4
ERROR_STEP      = 4
SHIFT_TYPE      = 'fft'                   # 'cubic' or 'fft'  (cubic hits CUFFT_INTERNAL_ERROR on 2592x2592 prefilter here)
PC_SIGMA        = 5.0                     # horizontal partial-coherence Gaussian σ [pixels]; 0 disables
REG_LAMBDA      = 3e-7                     # Tikhonov ||∇proj||² weight; 0 disables. Try 1e-4..1e-2.
LOG_LEVEL       = 'INFO'
...
# --- Geometry ---
wavelength    = 1.24e-9 / ENERGY                  # m (E in keV)
magnification = FOCUS_TO_DETECTOR_DISTANCE / Z1
voxelsize     = DETECTOR_PIXELSIZE / magnification
fresnel_z     = Z1 * Z2 / FOCUS_TO_DETECTOR_DISTANCE  # effective propagation distance
...
# --- Positions (mm -> m -> pixels) ---
tom_x_mm_all, tom_y_mm_all = read_positions(DATA_SCAN)

# --- Select subset of frames (identical selection on all ranks): N closest to (0,0) ---
...
spx = tom_x_mm * 1e-3   # horizontal (columns)
spy = tom_y_mm * 1e-3   # vertical   (rows)
# NFP convention in this codebase: pos[:, 0] = row (y), pos[:, 1] = col (x)
...
if rank == 0:
    ...
    logger.info(f'ntheta used             = {ntheta}  (N_POSITIONS={N_POSITIONS}, closest to (0,0))')
    if ntheta != ntheta_all:
        logger.info(f'selected indices        = {sel.tolist()}')
    logger.info(f'positions (mm)          : x in [{tom_x_mm.min():.4f}, {tom_x_mm.max():.4f}], '
...
local_sel = sel[cl.st_theta:cl.end_theta].tolist()   # h5py needs a plain list
with h5py.File(orca_path(DATA_SCAN), 'r') as f:
    raw = f['entry/instrument/orca/data'][local_sel,
                                          sty:sty+n, stx:stx+n].astype('float32')

corr = (raw - dark_mean) / denom          # flat-field-corrected intensity, ~1
