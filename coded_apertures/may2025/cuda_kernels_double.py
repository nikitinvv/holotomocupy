...
Efast_kernel = cp.RawKernel(
    r"""                              
extern "C" 
void __global__ Efast(double2* res, double2 *psi, int* stx, int* sty, int npos, int npatch, int npsi)
{
...
ETfast_kernel = cp.RawKernel(
    r"""                              
extern "C" 
void __global__ ETfast(double2* res, double2 *psi, int* stx, int* sty, int npos, int npatch, int npsi)
{
    int tx = blockDim.x * blockIdx.x + threadIdx.x;
    int ty = blockDim.y * blockIdx.y + threadIdx.y;
    int tz = blockDim.z * blockIdx.z + threadIdx.z;
...
extern "C" __global__ void __global__ wrap(double2 *f, int n, int nz, int m)
{
	int tx = blockDim.x * blockIdx.x + threadIdx.x;
	int ty = blockDim.y * blockIdx.y + threadIdx.y;
	int tz = blockDim.z * blockIdx.z + threadIdx.z;
...
extern "C" __global__ void wrapadj(double2 *f, int n, int nz, int m)
{
  int tx = blockDim.x * blockIdx.x + threadIdx.x;
  int ty = blockDim.y * blockIdx.y + threadIdx.y;
  int tz = blockDim.z * blockIdx.z + threadIdx.z;
...
extern "C" __global__ void gather(double2 *g, double2 *f, double *theta, int m,
                       double *mu, int n, int ntheta, int nz, bool direction)
{
  ...
  if (tx >= n || ty >= ntheta || tz >= nz)
    return;
  double M_PI = 3.141592653589793238f;
  double2 g0, g0t;
  double w, coeff0;
  double w0, w1, x0, y0, coeff1;
  ...
  x0 = (tx - n / 2) / (double)n * __cosf(theta[ty]);
  y0 = -(tx - n / 2) / (double)n * __sinf(theta[ty]);
  if (x0 >= 0.5f)
    x0 = 0.5f - 1e-5;
  if (y0 >= 0.5f)
    y0 = 0.5f - 1e-5;
  for (int i1 = 0; i1 < 2 * m + 1; i1++)
  {
    ...
    for (int i0 = 0; i0 < 2 * m + 1; i0++)
    {
      ...
      w0 = ell0 / (double)(2 * n) - x0;
      w1 = ell1 / (double)(2 * n) - y0;
...
extern "C" 
void __global__ pad_sym(double2* g, double2 *f, int pad_width, int n0, int n1, int ntheta, bool direction)
{
  int tx = blockDim.x * blockIdx.x + threadIdx.x;
  int ty = blockDim.y * blockIdx.y + threadIdx.y;
  int tz = blockDim.z * blockIdx.z + threadIdx.z;
...
extern "C" void __global__ pad(double2* g, double2 *f, int n, int ntheta, bool direction)
{
  int tx = blockDim.x * blockIdx.x + threadIdx.x;
  int ty = blockDim.y * blockIdx.y + threadIdx.y;
  int tz = blockDim.z * blockIdx.z + threadIdx.z;
...
extern "C" __global__ void gather_mag(double2 *g, double2 *f, double *magnification, int m,
                       double *mu, int n, int ne, int ntheta, bool direction)
{
  ...
  coeff0 = M_PI / mu[0];
  coeff1 = -M_PI * M_PI / mu[0];
  double s =  - (magnification[0]-1)*0.5-magnification[0]*(n/(double)ne-1)*0.5;
  x0 = -(tx - n / 2 + s) / (double)n / magnification[0];
  y0 = -(ty - n / 2 + s) / (double)n / magnification[0];
  ...
  for (int i1 = 0; i1 < 2 * m + 1; i1++)
  {
    ...
    for (int i0 = 0; i0 < 2 * m + 1; i0++)
    {
      ...
      if (ell0+m+ne<0 || ell0+m+ne>2*ne+2*m)
        continue;
      w0 = ell0 / (double)(2 * ne) - x0;
      w1 = ell1 / (double)(2 * ne) - y0;
      w = coeff0 * __expf(coeff1 * (w0 * w0 + w1 * w1)); 
