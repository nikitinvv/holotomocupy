...
class Tomo():    
    ...
    def R(self, obj, out=None):
        ...
        if out is None:
            data = cp.empty([self.ntheta, nz,n], dtype="complex64")
        else:
            data = out
        ...
        # STEP2: fft 2d
        wrap_kernel(
            (int(cp.ceil((2 * n + 2 * m) / 32)), int(cp.ceil((2 * n + 2 * m) / 32)), nz), (32, 32, 1), (fde, n, nz, m)
        )
    ...
    def RT(self, data):
        ...
        nz,n = data.shape[1:]
        obj = cp.empty([data.shape[1], self.n, self.n], dtype="complex64")
