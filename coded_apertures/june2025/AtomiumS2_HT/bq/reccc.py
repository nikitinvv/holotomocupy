...
class Rec:
    ...
    def rec_tomo(self,d,niter=1):
        ...
        for k in range(niter):
            tmp=self.linear_batch(Ru,d,2,-2,self.pool_cpu)
            grad = self.RT(tmp)            
            Rgrad = self.R(grad)            
            if k==0:
                ...
                beta = beta.get()
                eta=self.linear_batch(grad,eta,-1,beta,self.pool_cpu)
                Reta = self.linear_batch(Rgrad,Reta,-1,beta,self.pool_cpu)
            alpha = -self.redot_batch(grad,eta)/(2*self.redot_batch(Reta,Reta))
            alpha = alpha.get()
            u = self.linear_batch(u,eta,1,alpha,self.pool_cpu)
            Ru = self.linear_batch(Ru,Reta,1,alpha,self.pool_cpu)            
        return u
