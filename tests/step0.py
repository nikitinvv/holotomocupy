...
from holotomocupy.rec_nfp_mpi_2x import RecNFP2x
...
parser = argparse.ArgumentParser(prog='BH-nfp',
                    description='Reconstruct the nfp with bh-ptycho')

parser.add_argument("filename", type=str, help="Config filename")
parser.add_argument("--egy", type=float, default=None, help="Override energy")

pargs = parser.parse_args()

args = parse_args_step0_nx(pargs.filename)
...
niter    = args.niter
nchunk   = args.nchunk
vis_step = args.checkpoint_step
err_step = args.error_step
rho      = args.rho
shift_type = args.shift_type
shift_symmetric = args.shift_symmetric
...
energy     = pargs.egy or meta['energy']
z1         = meta['z1']
...
_path_out = os.path.join(_path_out, f"egy_{energy:.03f}keV")
...
if rank==0:
  logger.warning(f'{shift_type=}')
  logger.warning(f'{shift_symmetric=}')

cl = RecNFP2x(rec_args)
...
#cl.vars['pos'][...,0]+=float(sys.argv[1])
#cl.vars['pos'][...,1]=cp.round(cl.vars['pos'][...,1])
#cl.vars['pos'][...,1]+=0.9#cp.round(cl.vars['pos'][...,1])



print(cl.vars['pos'])

#err = (cp.random.random(cl.vars['pos'].shape)-0.5)*0.5
#cl.vars['pos']+=err
#logger.warning(f"  pos err y: {np.array2string(err[:, 0].get(), precision=4, separator=', ')}")
#logger.warning(f"  pos err x: {np.array2string(err[:, 1].get(), precision=4, separator=', ')}")
...
if rank == 0:
    ...
    with h5py.File(os.path.join(_path_out, os.path.basename(h5_out)), 'w') as f:
